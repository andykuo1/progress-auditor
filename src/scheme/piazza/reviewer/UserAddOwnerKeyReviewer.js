...
export async function review(db, config, reviewID, reviewType, reviewParams)
{
    ...
    if (reviewParams.length < 2) db.throwError(ERROR_TAG, `Missing review params - expected 2 parameter.`, { id: [reviewID, reviewType], options: [`Add more parameters to the review.`] });

    const user = UserDatabase.getUserByID(db, reviewParams[0]);
    if (!user)
    {
        db.throwError(ERROR_TAG, `Invalid review param - Cannot find user for id '${reviewParams[0]}'.`, {
