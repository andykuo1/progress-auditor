...
const { parseEmail } = require('../FieldParser.js');

const SubmissionDatabase = require('../SubmissionDatabase.js');
const { SUBMISSION_TYPE_UNKNOWN,
    SUBMISSION_TYPE_SOURCE,
    SUBMISSION_TYPE_MAJOR_EDIT,
    SUBMISSION_TYPE_MINOR_EDIT } = require('../Submission.js');
...
async function parse(filepath, db, opts={})
{
    ...
    await readCSVFileByRow(filepath, (row) => {
        ...
        try
        {
            ...
            const submission = SubmissionDatabase.addSubmission(db, ownerKey, assignmentID, submitDate, attributes);
        }
        catch(e)
        {
