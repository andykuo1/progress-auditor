...
function generateUserReport(db, userID)
{
    ...
    printRow('Start: ' + dateToString(userData[1]), LENGTH);
    printRow('  End: ' + dateToString(userData[2]), LENGTH);
    printRow('Slips: ' + calculateNumberOfSlipDays(schedule), LENGTH)
    printDivider(LENGTH);
    console.log();

    generateUserSchedule(db, schedule);
    console.log();
    generateSubmissionReport(db, userData);
    console.log();
}

function generateSubmissionReport(db, userData)
{
    const submissions = [
        ['intro', new Date('July 06, 2019')],
        ['week1', new Date('July 14, 2019')],
        ['week2', new Date('July 21, 2019')],
        ['week3', new Date('July 28, 2019')],
        ['week4', new Date('Aug 04, 2019')],
        ['week5', new Date('Aug 11, 2019')],
        // ['week10', new Date('July 07, 2019')],
        ['last', new Date('Sep 22, 2019')]
    ];

    // Compute statuses for each submission...
}

function generateUserSchedule(db, schedule)
{
    ...
    for(const assignment of assignments)
    {
        ...
        dueDates.push((date.getMonth() + 1) + '/' + date.getDate());
    }
