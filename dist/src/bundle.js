 ...
 * @param {Object} opts Any additional options.
 */
async function parse$2(db, config, filepath, opts={ maxEndDates: [] })
{
    setupDatabase$1(db);

    const maxEndDateEntries = processMaxEndDateEntries$1(opts.maxEndDates);
    ...
    await readCSVFileByRow(filepath, (row) => {
        ...
        // 9. Which quarter will you enroll in CSE 197?
        // ...
        // 12. Start date of internship
        // 13. Anticipated end date of internship
        // ...
        // 24. EASy approval date
        // 25. Admin notes
        // 28. Max slip days allowed (giving 4 instead of 3 per week to account for time zone)
        ...
        {
            const userID = row[5].trim().toUpperCase(); // FieldParser.parseEmail(row[6]);
            const ownerKey = parseEmail(row[6], row[1], row[7]);
            const userName = parseName(`${row[3]} ${row[2]}`);
            const pid = row[5].trim().toUpperCase();
            const startDate = parseAmericanDate(row[12]);
            const endDate = parseAmericanDate(row[13]);
...
function processMaxEndDateEntries$1(maxEndDates)
{
    if (!Array.isArray(maxEndDates)) return [];
    if (maxEndDates.length <= 0) return [];
...
var CohortParser2020 = /*#__PURE__*/Object.freeze({
    PARSER_OPTIONS: PARSER_OPTIONS$1,
    parse: parse$2
});
