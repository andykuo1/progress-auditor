...
{
    divider();
    log(chalk.green(nodeFiglet.textSync(title, { font: 'Big' })));
    if (subtitle)
    {
...
function printToString(...messages)
{
...
function printTitle()
{
    doTheBigTitleThing(TITLE, 'Version ' + version);
}
...
async function askWhetherToReviewErrors(db, config, errors)
{
    ...
    printMotivation();
    const result = await ask("Do you want to review them now?");
    if (!result)
    {
...
async function askWhetherToMakeNewConfig()
{
    return await ask('Make a new config?');
}
...
{
    if (await askWhetherToMakeNewConfig())
    {
...
{
    await review$1(db, config, db[REVIEW_KEY]);
}
...
async function outputDebugLog(db, config)
{
    if (await askWhetherToSaveDebugInfo())
    {
        const outputPath = config.outputPath;
        const outputAutoDate = config.outputAutoDate || false;
        const filePath = outputPath + (outputAutoDate ? '/' + stringify(db.currentDate, false) : '');
...
async function validateDatabase(db, config)
{
    ...
    if (await shouldSaveNewReviewsForClient(db, config, reviews))
    {
        ...
        await outputNewReviewsToFile(db, config, null);
    }
...
async function onStart(directory, args)
{
    ...
    println("Running from directory:", directory);
    println();
}
